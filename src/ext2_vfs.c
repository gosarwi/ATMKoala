...
static vfs_fsops_t ops={.lookup=lookup,.readdir=readdir,.read=readop,.stat=statop};
int ext2_vfs_mount(const char *path){if(active||!ext2.mounted||!path||!path[0])return -1;kmemset(nodes,0,sizeof(nodes));ext2_vnode_t*root=get_node("/");if(!root)return -1;if(vfs_mount(path,&root->ino)<0)return -1;kstrncpy(mounted_path,path,VFS_PATH_MAX-1);active=1;return 0;}
int ext2_vfs_unmount(void){if(!active)return 0;int r=vfs_unmount(mounted_path);active=0;mounted_path[0]=0;kmemset(nodes,0,sizeof(nodes));return r;}
